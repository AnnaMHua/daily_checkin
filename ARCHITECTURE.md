...
这个项目是一个本机自动化工具，用 macOS LaunchAgent 定时驱动专属 Google Chrome profile，完成一亩三分地每日答题和每日签到。它不保存账号密码，不读取 Chrome cookie 文件，不绕过真人验证，也不调用 LLM；遇到登录失效、验证码或人机验证时会记录错误并保留 Chrome 窗口给用户手动处理。
...
| 情况 | 行为 |
| --- | --- |
| 未登录 / 真人验证 / 验证码 | 记录当前步骤需要人工处理，退出码为 `2` |
